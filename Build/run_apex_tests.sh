...
sf project deploy start --source-dir src --target-dev-hub DevHub

echo "Testing code in org"
sf apex run test --test-level RunLocalTests --outputdir test-results --result-format tap --target-dev-hub DevHub